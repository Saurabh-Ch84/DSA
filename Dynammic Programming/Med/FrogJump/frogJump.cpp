...
class Solution {
    ...
    int findAnswer1(vector<int> &n, int s, int e) {
        if (s == e) return 0;
        if (s > e) return 1e9; // Not needed in practice, but safe
        int jump1 = abs(n[s] - n[s+1]) + findAnswer1(n, s+1, e);
        int jump2 = (s+2 <= e) ? abs(n[s] - n[s+2]) + findAnswer1(n, s+2, e) : 1e9;
    ...
    int findAnswer2(vector<int> &n,int e){
        ...
        int jump2=(e>1)? abs(n[e]-n[e-2])+findAnswer2(n,e-2):1e9;
        return min(jump1,jump2);
    }
    ...
    int findAnswer3(vector<int> &n,vector<int> &dp,int e){
        if(e==0) return 0;
        if(e<0) return 1e9;
        if(dp[e]!=1e9) return dp[e];
        int jump1=abs(n[e]-n[e-1])+findAnswer3(n,dp,e-1);
        int jump2=(e>1)?abs(n[e]-n[e-2])+findAnswer3(n,dp,e-2):1e9;
