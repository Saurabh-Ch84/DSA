...
public:
    vector<int> largestDivisibleSubset(vector<int> &nums)
    {
        ...
        int globalMaxLength = 1, globalMaxIdx = 0;

        for (int i = n - 1; i >= 0; i--)
        {
            result[i].push_back(nums[i]);
            int j = i + 1;
            int localMaxLength = 0, localMaxIdx = i;
            while (j < n)
            {
                if (nums[j] % nums[i] == 0 && result[j].size() > localMaxLength)
                {
                    localMaxLength = result[j].size();
                    localMaxIdx = j;
                }
                j++;
            }
            if (localMaxIdx != i)
                result[i].insert(result[i].end(), result[localMaxIdx].begin(), result[localMaxIdx].end());

            if (result[i].size() > globalMaxLength)
            {
                globalMaxLength = result[i].size();
                globalMaxIdx = i;
            }
        }
        print(result);
        print(result[globalMaxIdx]);
        return result[globalMaxIdx];
    }
    vector<int> largestDivisibleSubset(vector<int> &nums){
        int m=nums.size();
         if (m <= 1) return nums;
